"""
TEST 2: Utterance Boundary Timeout Optimization
===============================================

PROBLEM: Even with high ASR confidence (0.92) and syntactically complete
utterances, the system waits a full 3 seconds before finalizing.

EVIDENCE FROM LOGS:
- 15:22:06,524 - utterance_schedule_finalize timeout_s=3.000
- 15:22:09,528 - utterance_finalize (3 seconds later!)

SOLUTION: Reduce timeout for high-confidence complete utterances:
- Confidence >= 0.90 AND syntactically complete → 1.5s timeout
- Confidence >= 0.75 AND speech_ended → 2.0s timeout
- Otherwise → keep current 3.0s timeout
...
class TestCurrentTimeoutBehavior:
    ...
    def test_document_current_high_confidence_timeout(self, decider):
        ...
        # The optimization tests below will enforce the target behavior
...
class TestOptimizedTimeoutBehavior:
    ...
    def test_high_confidence_complete_question_fast_timeout(self, decider):
        """
        OPTIMIZATION TARGET: High confidence complete question → short timeout.

        Current: 3.0s
        Target: 1.5s
        Savings: 1.5s per utterance
        """
        decision = decider.decide(
            text="What is the best paintbrush for detailed work?",
            confidence=0.92,
        ...
        # TARGET: Should be short_timeout (1.5s)
...
class TestTimeSavingsCalculation:
    ...
    def test_savings_per_utterance(self):
        ...
        current_timeout = 3.0  # Current behavior
    ...
    def test_percentage_of_utterances_optimized(self):
        ...
        # - ~70% of utterances are complete questions/commands
        # - ~80% of those have high confidence (>0.90)
        # - So ~56% of utterances should get faster timeout
