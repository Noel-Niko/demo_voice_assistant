...
## Table of Contents
- [Network Preconditions: vNext SNI Blocking & Verification](#network-preconditions-vnext-sni-blocking--verification)
...
## Network Preconditions: vNext SNI Blocking & Verification

Summary
- Root cause: SNI-based TLS inspection in the vNext path resets connections when SNI=`speech.googleapis.com`.
- Impact: Google Cloud Speech (and potentially TTS/auth) calls from the cluster fail. Local dev may work.
- Constraint: Google APIs require SNI; client libraries always send it; SNI cannot be disabled.

Required action
- Ask the Network/Security team (Zscaler/SSL inspection policy owners) to add an SNI/Server Name bypass for `speech.googleapis.com` on TCP 443 for the cluster egress IPs. Optionally include `oauth2.googleapis.com` if tokens are fetched at runtime.

How to verify (cluster)
1) Python SNI/no-SNI repro inside a pod
...
print('=' * 60)
print('PROOF: SNI-based blocking')
print('=' * 60)

# Test 1: No SNI - works
print('\n1. Connection WITHOUT SNI (no hostname in TLS):')
try:
...
# Test 2: With SNI for speech.googleapis.com - blocked
print('\n2. Connection WITH SNI=speech.googleapis.com:')
try:
    context = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
    context.check_hostname = False
    context.verify_mode = ssl.CERT_NONE
    with socket.create_connection((ip, 443), timeout=5) as sock:
        with context.wrap_socket(sock, server_hostname='speech.googleapis.com') as ssock:
            print('   ✓ SUCCESS')
except Exception as e:
    print(f'   ✗ BLOCKED: {e}')

# Test 3: With SNI for www.googleapis.com - test if all googleapis blocked
print('\n3. Connection WITH SNI=www.googleapis.com:')
try:
    context = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
    context.check_hostname = False
    context.verify_mode = ssl.CERT_NONE
...
print('\n' + '=' * 60)
print('CONCLUSION: SNI field is being inspected and blocked')
print('=' * 60)
"
```

2) OpenSSL quick check (optional)

```bash
# With SNI (expected to fail before fix)
openssl s_client -connect <ip>:443 -servername speech.googleapis.com -brief

# Without SNI (expected to succeed before fix)
openssl s_client -connect <ip>:443 -noservername -brief
```

Requesting
- Add SSL inspection “Do Not Inspect”/bypass based on SNI= `speech.googleapis.com` on TCP 443 for the cluster egress IPs (vNext).
- Rationale: TLS with SNI=`speech.googleapis.com` is reset; no-SNI succeeds; Google APIs require SNI; SDK always sends it.
- Optional: Include `oauth2.googleapis.com` if tokens are fetched server-side.
